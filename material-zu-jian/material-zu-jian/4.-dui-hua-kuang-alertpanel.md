...
_**介绍**_：`BottomSheet` 组件本身很少直接使用。相反，最好使用 `scaffold` 状态创建一个持久的底层表。`ScaffoldState.showBottomSheet` 或 `Scaffold.bottomSheet`, 和带有 `showmodalbottom` 表单的模态底部表单。
...
_**说明**_：具有可选操作的轻量级消息提示，在屏幕的底部显示。类似于 `toast`。

_**介绍**_：

* 带有可选操作的轻量级消息，该操作将在屏幕底部短暂显示。

* 要显示一个 `SnackBar`，调用 `Scaffold.of(context). showsnackbar()`，传递一个描述消息的 `SnackBar` 实例。
  
* 要控制 `SnackBar` 保持可见的时间，请指定持续时间。
