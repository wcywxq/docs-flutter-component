...
_**介绍**_：滑块可用于从连续或离散的一组值中进行选择。默认值是使用从最小到最大值的连续范围。若要使用离散值，请为除法使用非空值，该值表示离散区间的数量。例如，如果 _`min`_ 是 _`0.0`_, _`max`_ 是 _`50.0`_，除数是 _`5`_，那么滑块可以取离散值 _`0.0`_、_`10.0`_、_`20.0_`、 _`30.0`_、 _`40.0`_ 和 _`50.0`_。
